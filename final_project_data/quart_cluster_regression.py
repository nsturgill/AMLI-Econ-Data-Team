...
for quart, df_quart in data.groupby('full_quart_cluster'):

    x = (df_quart.iloc[:, 17:37])
    y = df_quart['Estimate; INCOME AND BENEFITS (IN 2017 INFLATION-ADJUSTED DOLLARS) - Total households - Median household income (dollars)']
    ...
    descdat[quart] = desclist

    
    #print(y.describe())
    print("\nStatistics for: ", quart)
...
descdat.to_csv(path_or_buf = "Dropbox/AMLI/final_project_data/quart_cluster_regression_results.csv")
